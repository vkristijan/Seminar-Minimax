 ...
 * order to calculate the best move possible. After reaching a certain depth of
 * the search tree, a {@link Heuristic} algorithm will be called to calculate
 * the best move.
 * 
 * @author Kristijan Vulinovic
 */
public class AiMinimax extends AbstractAi {
	/**
	 ...
	 */
	public AiMinimax(Heuristic heuristic, int depth) {
		super();
	...
	public int nextMove(List<Integer> moves, CellState currentPlayer) {
		...
		for (int i = 0; i < score.length; ++i) {
			...
			if (GameUtility.checkWinner(GameUtility.listToBoard(tmpMoves, width, height)) == currentPlayer) {
				...
				score[i] = min(tmpMoves, GameUtility.switchPlayer(currentPlayer), 1);
			}
