 ...
 * pruning</a> in order to calculate the best move possible. This implementation
 * is designed to work using multiple threads in orders to speed up the
 * calculation. After reaching a certain depth of the search tree, a
 * {@link Heuristic} algorithm will be called to calculate the best move.
 * 
 * @author Kristijan Vulinovic
 */
public class AiAlphaBetaPruningRandomMultithreaded extends AbstractAi {
	/**
	 ...
	 */
	public AiAlphaBetaPruningRandomMultithreaded(Heuristic heuristic, int depth) {
		super();
			...
			thread[i] = new Thread() {
				...
				public void run() {
					...
					if (GameUtility.checkWinner(GameUtility.listToBoard(tmpMoves, width, height)) == currentPlayer) {
						...
						score[iteration] = min(tmpMoves, GameUtility.switchPlayer(currentPlayer), 1, alpha, beta);
					}
				}
			};
			thread[i].start();
