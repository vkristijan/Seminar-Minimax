 ...
 */
public class AiAlphaBetaPruningRandom extends AbstractAi {
	/**
	 ...
	 */
	public AiAlphaBetaPruningRandom(Heuristic heuristic, int depth) {
		super();
	...
	public int nextMove(List<Integer> moves, CellState currentPlayer) {
		...
		for (int i = 0; i < score.length; ++i) {
			...
			if (GameUtility.checkWinner(GameUtility.listToBoard(tmpMoves, width, height)) == currentPlayer) {
				...
				score[i] = min(tmpMoves, GameUtility.switchPlayer(currentPlayer), 1, alpha, beta);
			}
